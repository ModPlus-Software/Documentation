...
| `{mps:Margin 0.25}` | `1 1 1 1` | `1 1 1 1` |
...
| `{mps:Margin '1 0.25 0.25 0.25'}` | `4 1 1 1` | `2 1 1 1` |

### Округление дробных множителей

Начиная с версии 16.0.10 ненулевой отступ не бывает меньше одного пикселя. Если дробный
множитель даёт значение по модулю меньше единицы (например, `0.25 × 2 = 0.5` в компактном
режиме), оно округляется до `1` с сохранением знака. Благодаря этому тонкий отступ
не пропадает и не размывается при переходе в компактный режим.

* Нулевой множитель по-прежнему даёт `0`.
* Отрицательный дробный отступ округляется до `-1`.
* Значения от одного пикселя и больше не меняются: `0.75 × 2 = 1.5` так и останется `1.5`.

Поэтому `0.25` — наименьший осмысленный множитель: в обычном режиме это `1`, в компактном
тоже `1`.
...
пересчитывается при переключении режима на лету. Округление дробных значений до одного
пикселя действует и здесь (см. [Округление дробных множителей](#округление-дробных-множителей)).
...
* Пересчёт множителей выполняет конвертер `DensityThicknessConverter`, он же приводит
  ненулевые значения меньше единицы к `±1`.
